...
class GameVM(
    ...
    fun nextWord(acierto : Boolean) {
        val inc = if (acierto) 1+uiState.value.streak else -1
        ...
        uiState.value.word?.let {
            _uiState.update { currentState ->
                currentState.copy(
                    ...
                    streak = if(acierto) currentState.streak+1 else 0,
                    maxStreak = if(acierto && currentState.maxStreak == currentState.streak) currentState.maxStreak+1 else currentState.maxStreak
                )
            }
